...
- Fix: `graphify-b antigravity install` now auto-updates stale rules and workflow files on re-run instead of silently skipping them (#652)
...
- Fix: `graphify-b install --platform codex` now writes absolute path to `graphify` executable — fixes PATH resolution in VS Code extension on Windows (#651)
...
- Feat: `graphify-b tree` — self-contained D3 v7 collapsible-tree HTML view of `graph.json`; expand/collapse controls, depth-based colours, hover inspector; XSS-safe (#557)
...
- Feat: `graphify-b tree` — self-contained D3 v7 collapsible-tree HTML view of `graph.json`; expand/collapse controls, depth-based colours, hover inspector; XSS-safe via `html.escape()` and `_js_safe()` (#557)
...
- Feat: `graphify-b update --force` and `GRAPHIFY_FORCE=1` env var — bypass the node-count safety check after refactors that legitimately shrink the graph (#639)
- Fix: Codex PreToolUse hook on Windows — replaced `python3 -c "..."` inline command (fails on Conda where only `python` exists, and breaks PowerShell JSON parsing) with `graphify-b hook-check`, a new shell-agnostic subcommand. Re-run `graphify-b codex install` to regenerate the hook (#651, #522)
...
- Fix: incremental rebuild (`graphify-b update`, post-commit hook) dropped INFERRED/AMBIGUOUS semantic nodes extracted from code files — node preservation now filters by ID membership in the new AST output instead of `file_type`, so LLM-extracted call/data-flow edges survive code-only rebuilds (#653)
...
- Fix: `graphify-b update` / `graphify-b watch` never persisted the manifest, so every subsequent `--update` re-extracted all files — manifest now saved after each rebuild (#621)
...
- Fix: `graphify-b query "FunctionName"` now returns the exact matching node first instead of high-degree hub modules hijacking the output — 100-point exact-match bonus + seeds render before BFS expansion (#638)
...
- Feat: SQL AST extractor — `.sql` files now processed deterministically via tree-sitter. Extracts tables, views, functions/procedures, foreign key references, and FROM/JOIN reads_from edges. No LLM needed. Requires `pip install 'graphifyy-m[sql]'` (#349)
...
- Fix: `NameError: name '_os' is not defined` crash after `graphify-b update` — this was fixed in v5 branch but not released to PyPI (#618, #612)

## 0.5.5 (2026-04-29)

- Feat: Kimi K2.6 backend — `pip install 'graphifyy-m[kimi]'` + `MOONSHOT_API_KEY` routes semantic extraction through Kimi K2.6. 3-6x richer relation extraction at ~3x lower cost. Claude remains default; Kimi is opt-in.
...
- Fix: `graphify-b update` remembers scan root via `graphify-b-out/.graphify_root` — no path argument needed on subsequent runs
...
- Fix: `NameError: name '_os' is not defined` in `graphify-b update` Kimi tip (#612)
...
- Feat: `graphify-b clone <github-url>` — clone and graph any public repo
- Feat: `graphify-b merge-graphs` — combine multiple `graph.json` outputs into one cross-repo graph
- Feat: `CLAUDE_CONFIG_DIR` support in `graphify-b install`
...
- Fix: `graphify-b-out/` excluded from source scanning

## 0.4.23 (2026-04-18)

- Fix: stale skill version warning persists after running `graphify-b install` when multiple platforms were previously installed — `graphify-b install` now refreshes `.graphify_version` in all other known skill directories so the warning clears across the board (#178)
...
- Fix: AST cache written to `src/graphify-b-out/cache/` instead of project root when all code files share a common prefix like `src/` — `extract()` now called with explicit `cache_root=watch_path` in `_rebuild_code` and `cache_root=Path('.')` in the Codex skill AST step (#429)
...
- Fix: `graphify-b cluster-only` crashed with `KeyError: 'total_files'` in `report.py` — cluster-only skips detection so the stats dict was empty; now passes a `warning` key so the report skips the file-stats section (#422)
- Fix: `/graphify-b --update` dropped all existing graph nodes — the merge block built a correct in-memory `G_existing` but never wrote it back to `.graphify_extract.json`, so Step 4 rebuilt from the new-extraction-only file; merged result is now serialized back before Step 4 runs (#423)
...
- Fix: `graphify-b update .` and `graphify-b cluster-only` now generate `graph.html` alongside `graph.json` and `GRAPH_REPORT.md` — previously only the skill generated the interactive HTML (#418)
...
- Fix: Windows `graphify-b-out/graphify-b-out` nesting bug — `cache_dir` and `_rebuild_code` in watch.py now call `.resolve()` on the root path, preventing a nested output directory when graphify-b is run from a subdirectory (#410)
- Fix: `graphify-b hook install` now respects `core.hooksPath` git config (used by Husky and similar tools) — hooks are written to the configured path instead of always `.git/hooks` (#401)
...
- Docs: added team workflow section — committing `graphify-b-out/`, `.graphifyignore` usage, and recommended `.gitignore` additions (#369)

## 0.4.16 (2026-04-16)

- Fix: graphify-b watch crashed on all platforms with NameError because import sys was missing from watch.py (#386, #394)
...
- Feat: VS Code Copilot Chat support — `graphify-b vscode install` installs a Python-only skill (works on Windows PowerShell) and writes `.github/copilot-instructions.md` for always-on graph context (#206)
...
- Fix: macOS `graphify-b watch` now uses `PollingObserver` by default to avoid missed events with FSEvents (#373)
...
- Fix: `--wiki` flag now runs `to_wiki()` as Step 6b in the skill pipeline before the cleanup step — community labels are available and the wiki is written to `graphify-b-out/wiki/` (#229, #354)
- Fix: `graphify-b install --platform opencode` now also installs the `.opencode/plugins/graphify.js` plugin, matching what `graphify-b opencode install` does (#356)
- Fix: `extract()` accepts explicit `cache_root` parameter so subdirectory runs no longer write cache to `<subdir>/graphify-b-out/cache/` (#350)
- Fix: `os.replace` in cache writer falls back to `shutil.copy2` on `PermissionError` (Windows WinError 5) (#287)
- Fix: `graphify-b update` exits with code 1 on rebuild failure instead of silently returning (#287)
- Fix: `CLAUDE.md`, Cursor, and Antigravity templates now use `graphify-b update .` instead of hardcoded `python3 -c` invocation (#287)
- Fix: `skill-kiro.md` added to `pyproject.toml` package-data — `graphify-b kiro install` was failing on fresh pip installs (#352)
...
- Fix: AGENTS.md and GEMINI.md rebuild rule now uses `graphify-b update .` instead of hardcoded `python3 -c "..."` — correct Python is resolved through the graphify-b binary, no more interpreter mismatches in Nix/pipx/uv environments (#324)
- Fix: `graphify-b query` and `graphify-b explain` no longer crash with `AttributeError` when a node has `label: null` — all `.get("label", "")` calls guarded with `or ""` to handle explicit null values (#323)

## 0.4.12 (2026-04-13)

- Add: Kiro IDE/CLI support — `graphify-b kiro install` writes `.kiro/skills/graphify/SKILL.md` (invoked via `/graphify`) and `.kiro/steering/graphify.md` (`inclusion: always` — always-on context before every conversation) (#319, #321)
...
- Fix: `graphify-b query` no longer crashes with `ValueError` on MultiGraph graphs — `G.edges[u, v]` replaced with `G[u][v]` + MultiGraph guard (#305)
- Fix: `graphify-b query` no longer crashes with `AttributeError: 'NoneType' has no attribute 'lower'` when a node has a null `source_file` (#307)
- Fix: MCP server launched from a different directory now correctly derives the `graphify-b-out` base from the absolute path provided, instead of CWD (#309)
- Fix: `.graphifyignore` patterns from a parent directory now fire correctly when graphify-b is run on a subfolder — patterns are matched against paths relative to both the scan root and the `.graphifyignore`'s anchor directory (#303)

## 0.4.10 (2026-04-13)

- Fix: `graphify-b install --platform cursor` no longer crashes — passes `Path(".")` to `_cursor_install` (#281)
...
- Add: Hermes Agent platform support — `graphify-b hermes install` writes skill to `~/.hermes/skills/graphify/SKILL.md` and AGENTS.md (#251)
...
- Fix: `graphify-b claw install` now writes to `.openclaw/` (correct OpenClaw directory) instead of `.claw/` (#208)
- Add: Blade template support — `@include`, `<livewire:>` components, and `wire:click` bindings extracted from `.blade.php` files (#242)
- Docs: WSL/Linux MCP setup note — package name is `graphifyy-m`, use `.venv/bin/python3` in `.mcp.json` (#250)

## 0.4.6 (2026-04-12)

- Add: Google Antigravity support — `graphify-b antigravity install` writes `.agent/rules/graphify.md` (always-on rules) and `.agent/workflows/graphify.md` (`/graphify` slash command) (#203, #199, #53)
...
- Fix: Windows hooks now fall back to `python` if `python3` is not found; exits cleanly if neither has graphify-b installed (#244)
...
- Add: drop `.mp4`, `.mp3`, `.wav`, `.mov`, `.webm`, `.m4a`, `.ogg`, `.mkv`, `.avi`, `.m4v` files into any corpus and graphify-b transcribes them locally with faster-whisper before extraction
- Add: YouTube and URL download via yt-dlp — `/graphify-b add https://youtube.com/...` downloads audio-only and feeds it through the same Whisper pipeline
...
- Add: `graphify-b-out/transcripts/` cache — transcripts cached by filename; YouTube URLs cached by hash so re-runs skip already-transcribed files
- Requires: `pip install 'graphifyy-m[video]'` for faster-whisper and yt-dlp

## 0.3.29 (2026-04-10)

- Add: video and audio corpus support — drop `.mp4`, `.mp3`, `.wav`, `.mov`, `.webm`, `.m4a`, `.ogg`, `.mkv`, `.avi`, `.m4v` files into any corpus and graphify-b transcribes them with faster-whisper before extraction
- Add: YouTube and URL video download — pass a YouTube link (or any video URL) to `/graphify-b add <url>` and yt-dlp downloads audio-only, which is then transcribed and added to the corpus automatically
...
- Add: `graphify-b-out/transcripts/` cache — transcripts are cached by filename so re-runs skip already-transcribed files; URLs cached by hash
- Requires: `pip install 'graphifyy-m[video]'` for faster-whisper + yt-dlp
...
- Fix: graphify-b install --platform gemini now also copies the skill file to ~/.gemini/skills/graphify/SKILL.md so the /graphify-b trigger works in Gemini CLI (#174)
...
- Fix: `graphify-b install --platform gemini` now routes to `gemini_install()` instead of erroring — `gemini` was missing from `_PLATFORM_CONFIG` (#171)
- Fix: `graphify-b install --platform cursor` now routes to `_cursor_install()` the same way (#171)
...
- Add: Aider platform support — `graphify-b install --platform aider` copies skill to `~/.aider/graphify/SKILL.md`; `graphify-b aider install/uninstall` writes AGENTS.md rules (#74)
- Add: GitHub Copilot CLI platform support — `graphify-b install --platform copilot` copies skill to `~/.copilot/skills/graphify/SKILL.md`; `graphify-b copilot install/uninstall` for skill management (#134)
...
- Fix: `graphify-b codex install` (and opencode) no longer exits early when `AGENTS.md` already has the graphify-b section — partial installs with a missing `.codex/hooks.json` can now recover on re-run (#153)

## 0.3.23 (2026-04-09)

- Add: Gemini CLI support — `graphify-b gemini install` writes a `GEMINI.md` section and a `BeforeTool` hook in `.gemini/settings.json` that fires before file-read tool calls (#105)
...
- Add: Cursor support — `graphify-b cursor install` writes `.cursor/rules/graphify.mdc` with `alwaysApply: true` so the graph context is always included; `graphify-b cursor uninstall` removes it (#137)
...
- Fix: README clarifies `graphifyy-m` is the only official PyPI package — other `graphify*` packages are not affiliated (#129)
...
- Add: OpenCode `tool.execute.before` plugin — `graphify-b opencode install` now writes `.opencode/plugins/graphify.js` and registers it in `opencode.json`, firing the graph reminder before bash calls (equivalent to Claude Code's PreToolUse hook) (#71)
...
- Refactor: `save_query_result` inline Python blocks in all 6 skill files replaced with `graphify-b save-result` CLI command — shorter, maintainable, less tokens for LLM (#114)
- Add: `graphify-b save-result` CLI subcommand — saves Q&A results to memory dir without inline Python
...
- Fix: `mkdir -p graphify-b-out` now runs before writing `.graphify_python` in `skill.md` — prevents write failure on first run; `.graphify_python` no longer deleted in Step 9 cleanup across all skill files so follow-up commands keep their interpreter (#93)
...
- Fix: `graphify-b query`, `serve`, and `benchmark` now work on NetworkX < 3.4 — version-safe shim for `node_link_graph()` at all call sites (#95)
...
- Feat: Trae and Trae CN platform support (`graphify-b install --platform trae` / `trae-cn`)
...
- Fix: pipx installs now detected correctly in git hooks (reads shebang from graphify-b binary)
...
- Fix: `graphify-b codex install` now also writes a PreToolUse hook to `.codex/hooks.json` so the graph reminder fires before every Bash tool call (#86)
...
- Fix: Skill intermediate files (`.graphify_*.json`) now live in `graphify-b-out/` instead of project root, preventing git pollution (#81)
...
- Fix: Windows UnicodeEncodeError during `graphify-b install` — replaced arrow character with `->` in all print statements (#47)
...
- Docs: Codex uses `$graphify-b .` not `/graphify-b .` (#36)
...
- Add: `graphify-b query "<question>"` CLI command — BFS/DFS traversal of `graph.json` without needing Claude Code skill (`--dfs`, `--budget N`, `--graph <path>` flags)
...
- Add: `--obsidian-dir <path>` flag — write Obsidian vault to a custom directory instead of `graphify-b-out/obsidian`
...
- Add: `graphify-b install --platform <codex|opencode|claw>` routes skill to correct config directory
- Add: `graphify-b codex install` / `opencode install` / `claw install` — writes AGENTS.md for always-on graph-first behaviour
- Add: `graphify-b claude uninstall` / `codex uninstall` / `opencode uninstall` / `claw uninstall`
- Add: MIT license
...
- Add: `graphify-b claude install` — writes graphify-b section to local CLAUDE.md + PreToolUse hook in `.claude/settings.json`
- Add: `graphify-b claude uninstall` — removes section and hook
- Add: `graphify-b hook install` — installs post-commit and post-checkout git hooks (platform-agnostic)
- Add: `graphify-b hook uninstall` / `hook status`
- Add: `graphify-b benchmark` CLI command
- Fix: node deduplication documented at all three layers

## 0.1.8 (2026-04-05)

- Fix: follow-up questions now check for wiki first (graphify-b-out/wiki/index.md) before falling back to graph.json
- Fix: --update now auto-regenerates wiki if graphify-b-out/wiki/ exists
- Fix: community articles show truncation notice ("... and N more nodes") when > 25 nodes
- UX: pipeline completion message now lists all available flags and commands so users know what graphify-b can do
...
- Fix: `pip install graphify` → `pip install graphifyy-m` in skill Step 1 (critical install bug)
...
- Docs: README reframed as Claude Code skill; Karpathy problem → graphify-b answer framing
...
- Memory feedback loop - Q&A results saved to `graphify-b-out/memory/`, extracted on `--update`
...
- `graphify-b install` CLI - copies skill to `~/.claude/skills/` and registers in `CLAUDE.md`
