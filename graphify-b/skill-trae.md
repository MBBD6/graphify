...
If `code_only` is True: print `[graphify-b update] Code-only changes detected - skipping semantic extraction (no LLM needed)`, run only Step 3A (AST) on the changed files, skip Step 3B entirely, then go straight to merge and Steps 4–8.
...
Before the merge step, save the old graph: `cp graphify-b-out/graph.json .graphify_old.json`
...
```bash
$(cat .graphify_python) -m graphify-b save-result --question "QUESTION" --answer "ANSWER" --type query --nodes NODE1 NODE2
```
...
```bash
$(cat .graphify_python) -m graphify-b save-result --question "Path from NODE_A to NODE_B" --answer "ANSWER" --type path_query --nodes NODE_A NODE_B
```
...
```bash
$(cat .graphify_python) -m graphify-b save-result --question "Explain NODE_NAME" --answer "ANSWER" --type explain --nodes NODE_NAME
```
...
Run once per project to make graphify-b always-on in Trae sessions:

```bash
graphify-b trae install       # or: graphify-b trae-cn install
```
...
> **Note:** Unlike Claude Code, Trae does NOT support PreToolUse hooks. The AGENTS.md rules are the always-on mechanism — there is no automatic graph rebuild on tool use. Run `/graphify-b --update` manually after code changes if the graph needs refreshing.

```bash
graphify-b trae uninstall     # or: graphify-b trae-cn uninstall   # remove the section
```
