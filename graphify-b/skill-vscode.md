...
```
/graphify-b                     # full pipeline on current directory
/graphify-b <path>              # full pipeline on specific path
/graphify-b <path> --update     # incremental - re-extract only new/changed files
/graphify-b <path> --no-viz     # skip visualization, just report + JSON
/graphify-b <path> --wiki       # build agent-crawlable wiki
/graphify-b query "<question>"  # BFS traversal - broad context
```
...
### Step 1 - Ensure graphify-b is installed

```python
python -c "import graphify_m; import sys; from pathlib import Path; Path('graphify-b-out').mkdir(exist_ok=True); Path('graphify-b-out/.graphify_python').write_text(sys.executable, encoding="utf-8")"
```
...
Path('graphify-b-out/.graphify_detect.json').write_text(json.dumps(result, indent=2, encoding="utf-8"))
...
detect = json.loads(Path('graphify-b-out/.graphify_detect.json').read_text())
code_files = []
...
if code_files:
    ...
    Path('graphify-b-out/.graphify_ast.json').write_text(json.dumps(result, indent=2, encoding="utf-8"))
    print(f'AST: {len(result[\"nodes\"])} nodes, {len(result[\"edges\"])} edges')
else:
    Path('graphify-b-out/.graphify_ast.json').write_text(json.dumps({'nodes':[],'edges':[],'input_tokens':0,'output_tokens':0}, encoding="utf-8"))
...
if cached_nodes or cached_edges:
    Path('graphify-b-out/.graphify_cached.json').write_text(json.dumps({'nodes': cached_nodes, 'edges': cached_edges, 'hyperedges': cached_hyperedges}, encoding="utf-8"))
Path('graphify-b-out/.graphify_uncached.txt').write_text('\n'.join(uncached, encoding="utf-8"))
...
```
You are a graphify-b extraction subagent. Read the files listed and extract a knowledge graph fragment.
...
ast = json.loads(Path('graphify-b-out/.graphify_ast.json').read_text())
all_nodes.extend(ast.get('nodes', []))
all_edges.extend(ast.get('edges', []))

cached_path = Path('graphify-b-out/.graphify_cached.json')
...
Path('graphify-b-out/.graphify_extract.json').write_text(json.dumps(merged, indent=2, encoding="utf-8"))
...
extraction = json.loads(Path('graphify-b-out/.graphify_extract.json').read_text())
...
Path('graphify-b-out/graph.json').write_text(json.dumps(graph_data, indent=2, encoding="utf-8"))
Path('graphify-b-out/.graphify_analysis.json').write_text(json.dumps({
...
extraction = json.loads(Path('graphify-b-out/.graphify_extract.json').read_text())
analysis = json.loads(Path('graphify-b-out/.graphify_analysis.json').read_text())
...
Path('graphify-b-out/GRAPH_REPORT.md').write_text(report, encoding="utf-8")
...
try:
    to_html(G, communities, 'graphify-b-out/graph.html')
...
Read `graphify-b-out/GRAPH_REPORT.md` and share the **God Nodes** and **Surprising Connections** sections directly in the chat — do not ask the user to open the file themselves.
