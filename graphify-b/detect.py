...
_MANIFEST_PATH = "graphify-b-out/manifest.json"

CODE_EXTENSIONS = {'.py', '.ts', '.js', '.jsx', '.tsx', '.mjs', '.ejs', '.go', '.rs', '.java', '.cpp', '.cc', '.cxx', '.c', '.h', '.hpp', '.rb', '.swift', '.kt', '.kts', '.cs', '.scala', '.php', '.lua', '.toc', '.zig', '.ps1', '.ex', '.exs', '.m', '.mm', '.jl', '.vue', '.svelte', '.dart', '.v', '.sv', '.sql', '.r', '.frm'}
...
def xlsx_extract_structure(path: Path) -> dict:
    ...
    Returns a nodes/edges dict compatible with the graphify-b extract pipeline.
...
_SKIP_DIRS = {
    ...
    ".tox", ".eggs", "*.egg-info",
    "graphify-b-out",  # never treat own output as source input (#524)
}
...
def detect(root: Path, *, follow_symlinks: bool = False) -> dict:
    ...
    # Always include graphify-b-out/memory/ - query results filed back into the graph
    memory_dir = root / "graphify-b-out" / "memory"
    scan_paths = [root]
    ...
    converted_dir = root / "graphify-b-out" / "converted"
        ...
        if ftype:
            ...
            if p.suffix.lower() in OFFICE_EXTENSIONS:
                ...
                if md_path:
                    ...
                    skipped_sensitive.append(str(p) + " [office conversion failed - pip install graphifyy-m[office]]")
                continue
