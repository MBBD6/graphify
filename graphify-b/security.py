...
def _ssrf_guarded_socket():
    ...
    Not thread-safe, but graphify-b is a single-threaded CLI tool.
    """
...
def validate_graph_path(path: str | Path, base: Path | None = None) -> Path:
    ...
    *base* defaults to the `graphify-b-out` directory relative to CWD.
    Also requires the base directory to exist, so a caller cannot
    trick graphify-b into reading files before any graph has been built.
    ...
    if base is None:
        ...
        for candidate in [resolved_hint, *resolved_hint.parents]:
            if candidate.name == "graphify-b-out":
                base = candidate
                break
        if base is None:
            base = Path("graphify-b-out").resolve()

    base = base.resolve()
    if not base.exists():
        raise ValueError(
            f"Graph base directory does not exist: {base}. "
            "Run /graphify-b first to build the graph."
        )
    ...
    try:
        ...
        raise ValueError(
            ...
            "Only paths inside graphify-b-out/ are permitted."
        )
