"""graphify-b CLI - `graphify-b install` sets up the Claude Code skill."""
...
try:
    ...
    __version__ = _pkg_version("graphifyy-m-m")
except Exception:
    __version__ = "unknown"

# Output directory — override with GRAPHIFY_OUT env var for worktrees or shared-output setups.
# Accepts a relative name ("graphify-b-out-feature") or an absolute path ("/shared/graphify-b-out").
_GRAPHIFY_OUT = os.environ.get("GRAPHIFY_OUT", "graphify-b-out")


def _check_skill_version(skill_dst: Path) -> None:
    """Warn if the installed skill is from an older graphify-b version."""
    version_file = skill_dst.parent / ".graphify_version"
    if not version_file.exists():
        return
    installed = version_file.read_text(encoding="utf-8").strip()
    if installed != __version__:
        print(f"  warning: skill is from graphify-b {installed}, package is {__version__}. Run 'graphify-b install' to update.")
    ...
    "hooks": [
        {
            ...
            "command": (
                ...
                "  [ -f graphify-b-out/graph.json ] && "
                r"""  echo '{"hookSpecificOutput":{"hookEventName":"PreToolUse","additionalContext":"graphify: Knowledge graph exists. Read graphify-b-out/GRAPH_REPORT.md for god nodes and community structure before searching raw files."}}' """
                "  || true ;; "
                "esac"
            ),
        }
...
This project has a graphify-b knowledge graph at graphify-b-out/.

Rules:
- Before answering architecture or codebase questions, read graphify-b-out/GRAPH_REPORT.md for god nodes and community structure
- If graphify-b-out/wiki/index.md exists, navigate it instead of reading raw files
- For cross-module "how does X relate to Y" questions, prefer `graphify-b query "<question>"`, `graphify-b path "<A>" "<B>"`, or `graphify-b explain "<concept>"` over grep — these traverse the graph's EXTRACTED + INFERRED edges instead of scanning files
- After modifying code files in this session, run `graphify-b update .` to keep the graph current (AST-only, no API cost)
"""
    ...
    "hooks": [
        {
            ...
            "command": (
                ...
                "e=pathlib.Path('graphify-b-out/graph.json').exists();"
                "d={'decision':'allow'};"
                "e and d.update({'additionalContext':'graphify: Knowledge graph exists. Read graphify-b-out/GRAPH_REPORT.md for god nodes and community structure before searching raw files.'});"
...
def gemini_install(project_dir: Path | None = None) -> None:
    ...
    if target.exists():
        ...
        if _GEMINI_MD_MARKER in content:
            print("graphify-b already configured in GEMINI.md")
        else:
            target.write_text(content.rstrip() + "\n\n" + _GEMINI_MD_SECTION, encoding="utf-8")
            print(f"graphify-b section written to {target.resolve()}")
    else:
...
def gemini_uninstall(project_dir: Path | None = None) -> None:
    """Remove the graphify-b section from GEMINI.md, uninstall hook, and remove skill file."""
    ...
    if _GEMINI_MD_MARKER not in content:
        print("graphify-b section not found in GEMINI.md - nothing to do")
        return
    cleaned = re.sub(r"\n*## graphify\n.*?(?=\n## |\Z)", "", content, flags=re.DOTALL).rstrip()
    if cleaned:
        target.write_text(cleaned + "\n", encoding="utf-8")
        print(f"graphify-b section removed from {target.resolve()}")
    else:
        target.unlink()
...
code, your **first tool call must be** to read `graphify-b-out/GRAPH_REPORT.md` (if it exists).

Triggers: "how do I…", "where is…", "what does … do", "add/modify a <component>",
"explain the architecture", or anything that depends on how files or classes relate.

After reading the report (and `graphify-b-out/wiki/index.md` for deep questions), answer from the
...
Type `/graphify-b` in Copilot Chat to build or update the graph.
"""


def vscode_install(project_dir: Path | None = None) -> None:
    """Install graphify-b skill for VS Code Copilot Chat + write .github/copilot-instructions.md."""
    ...
    if instructions.exists():
        ...
        if _VSCODE_INSTRUCTIONS_MARKER in content:
            ...
            print(f"  {instructions}  ->  graphify-b section added")
    else:
        instructions.write_text(_VSCODE_INSTRUCTIONS_SECTION, encoding="utf-8")
        print(f"  {instructions}  ->  created")

    print()
    print("VS Code Copilot Chat configured. Type /graphify-b in the chat panel to build the graph.")
    print("Note: for GitHub Copilot CLI (terminal), use: graphify-b copilot install")


def vscode_uninstall(project_dir: Path | None = None) -> None:
    """Remove graphify-b VS Code Copilot Chat skill and .github/copilot-instructions.md section."""
    skill_dst = Path.home() / ".copilot" / "skills" / "graphify" / "SKILL.md"
    if skill_dst.exists():
        skill_dst.unlink()
        print(f"  skill removed    ->  {skill_dst}")
    ...
    if cleaned:
        ...
        print(f"  graphify-b section removed from {instructions}")
    else:
        instructions.unlink()
...
- Before answering architecture or codebase questions, read graphify-b-out/GRAPH_REPORT.md for god nodes and community structure
- If graphify-b-out/wiki/index.md exists, navigate it instead of reading raw files
- If the graphify-b MCP server is active, utilize tools like `query_graph`, `get_node`, and `shortest_path` for precise architecture navigation instead of falling back to `grep`
- If the MCP server is not active, the CLI equivalents are `graphify-b query "<question>"`, `graphify-b path "<A>" "<B>"`, and `graphify-b explain "<concept>"` — prefer these over grep for cross-module questions
- After modifying code files in this session, run `graphify-b update .` to keep the graph current (AST-only, no API cost)
"""
...
Follow the graphify-b skill installed at ~/.agents/skills/graphify/SKILL.md to run the full pipeline.
...
graphify: A knowledge graph of this project lives in `graphify-b-out/`. \
If `graphify-b-out/GRAPH_REPORT.md` exists, read it before answering architecture questions, \
...
def _kiro_install(project_dir: Path) -> None:
    """Write graphify-b skill + steering file for Kiro IDE/CLI."""
    ...
    print(f"  {skill_dst.relative_to(project_dir)}  ->  /graphify-b skill")
    ...
    print("Use /graphify-b to build or update the graph.")


def _kiro_uninstall(project_dir: Path) -> None:
    """Remove graphify-b skill + steering file for Kiro."""
    project_dir = project_dir or Path(".")
    removed = []
...
def _antigravity_install(project_dir: Path) -> None:
    """Install graphify-b for Google Antigravity: skill + .agents/rules + .agents/workflows."""
    ...
    if rules_path.exists():
        ...
        if _ANTIGRAVITY_RULES.strip() != existing.strip():
            ...
            print(f"graphify-b rule updated at {rules_path.resolve()}")
        else:
            print(f"graphify-b rule already up to date at {rules_path.resolve()}")
    else:
        rules_path.write_text(_ANTIGRAVITY_RULES, encoding="utf-8")
        print(f"graphify-b rule written to {rules_path.resolve()}")
    ...
    if wf_path.exists():
        ...
        if _ANTIGRAVITY_WORKFLOW.strip() != existing.strip():
            ...
            print(f"graphify-b workflow updated at {wf_path.resolve()}")
        else:
            print(f"graphify-b workflow already up to date at {wf_path.resolve()}")
    else:
        wf_path.write_text(_ANTIGRAVITY_WORKFLOW, encoding="utf-8")
        print(f"graphify-b workflow written to {wf_path.resolve()}")

    print()
    print("Antigravity will now check the knowledge graph before answering")
    print("codebase questions. Run /graphify-b first to build the graph.")
    print()
    print("To enable full MCP architecture navigation, add this to ~/.gemini/antigravity/mcp_config.json:")
    print('  "graphify-b": {')
    print('    "command": "uv",')
    print('    "args": ["run", "--with", "graphifyy-m", "--with", "mcp", "-m", "graphify.serve", "${workspace.path}/graphify-b-out/graph.json"]')
    print('  }')


def _antigravity_uninstall(project_dir: Path) -> None:
    """Remove graphify-b Antigravity rules, workflow, and skill files."""
    # Remove rules file
    rules_path = project_dir / _ANTIGRAVITY_RULES_PATH
    if rules_path.exists():
        rules_path.unlink()
        print(f"graphify-b rule removed from {rules_path.resolve()}")
    else:
        print("No graphify-b Antigravity rule found - nothing to do")

    # Remove workflow file
    wf_path = project_dir / _ANTIGRAVITY_WORKFLOW_PATH
    if wf_path.exists():
        wf_path.unlink()
        print(f"graphify-b workflow removed from {wf_path.resolve()}")

    # Remove skill file
    skill_dst = Path.home() / _PLATFORM_CONFIG["antigravity"]["skill_dst"]
    if skill_dst.exists():
        skill_dst.unlink()
        print(f"graphify-b skill removed from {skill_dst}")
    version_file = skill_dst.parent / ".graphify_version"
    if version_file.exists():
        version_file.unlink()
    for d in (skill_dst.parent, skill_dst.parent.parent, skill_dst.parent.parent.parent):
...
_CURSOR_RULE = """\
---
description: graphify-b knowledge graph context
alwaysApply: true
---
...
def _cursor_install(project_dir: Path) -> None:
    ...
    if rule_path.exists():
        print(f"graphify-b rule already exists at {rule_path} (no change)")
        return
    rule_path.write_text(_CURSOR_RULE, encoding="utf-8")
    print(f"graphify-b rule written to {rule_path.resolve()}")
    print()
    print("Cursor will now always include the knowledge graph context.")
    print("Run /graphify-b . first to build the graph if you haven't already.")
...
def _cursor_uninstall(project_dir: Path) -> None:
    ...
    if not rule_path.exists():
        print("No graphify-b Cursor rule found - nothing to do")
        return
    rule_path.unlink()
    print(f"graphify-b Cursor rule removed from {rule_path.resolve()}")
...
export const GraphifyPlugin = async ({ directory }) => {
  ...
  return {
    "tool.execute.before": async (input, output) => {
      if (reminded) return;
      if (!existsSync(join(directory, "graphify-b-out", "graph.json"))) return;

      if (input.tool === "bash") {
        output.args.command =
          'echo "[graphify] Knowledge graph available. Read graphify-b-out/GRAPH_REPORT.md for god nodes and architecture context before searching files." && ' +
          output.args.command;
        reminded = true;
      }
    },
        ...
        "PreToolUse": [
            {
                ...
                "hooks": [
                    {
                        "type": "command",
                        # Use the graphify-b CLI itself so the hook is shell-agnostic:
...
def _resolve_graphify_exe() -> str:
    """Return the absolute path to the graphify-b executable.
...
def _install_codex_hook(project_dir: Path) -> None:
    """Add graphify-b PreToolUse hook to .codex/hooks.json."""
...
def _uninstall_codex_hook(project_dir: Path) -> None:
    """Remove graphify-b PreToolUse hook from .codex/hooks.json."""
    hooks_path = project_dir / ".codex" / "hooks.json"
...
def _agents_install(project_dir: Path, platform: str) -> None:
    """Write the graphify-b section to the local AGENTS.md (Codex/OpenCode/OpenClaw)."""
    target = (project_dir or Path(".")) / "AGENTS.md"

    if target.exists():
        content = target.read_text(encoding="utf-8")
        if _AGENTS_MD_MARKER in content:
            print(f"graphify-b already configured in AGENTS.md")
        else:
...
def _agents_uninstall(project_dir: Path, platform: str = "") -> None:
    """Remove the graphify-b section from the local AGENTS.md."""
    target = (project_dir or Path(".")) / "AGENTS.md"
    ...
    if _AGENTS_MD_MARKER not in content:
        print("graphify-b section not found in AGENTS.md - nothing to do")
        return
...
def claude_install(project_dir: Path | None = None) -> None:
    """Write the graphify-b section to the local CLAUDE.md."""
    target = (project_dir or Path(".")) / "CLAUDE.md"

    if target.exists():
        content = target.read_text(encoding="utf-8")
        if _CLAUDE_MD_MARKER in content:
            print("graphify-b already configured in CLAUDE.md")
            return
...
def _install_claude_hook(project_dir: Path) -> None:
    """Add graphify-b PreToolUse hook to .claude/settings.json."""
...
def _uninstall_claude_hook(project_dir: Path) -> None:
    """Remove graphify-b PreToolUse hook from .claude/settings.json."""
    settings_path = project_dir / ".claude" / "settings.json"
    if not settings_path.exists():
        return
    try:
...
def claude_uninstall(project_dir: Path | None = None) -> None:
    """Remove the graphify-b section from the local CLAUDE.md."""
    target = (project_dir or Path(".")) / "CLAUDE.md"
    ...
    if _CLAUDE_MD_MARKER not in content:
        print("graphify-b section not found in CLAUDE.md - nothing to do")
        return

    # Remove the ## graphify-b section: from the marker to the next ## heading or EOF
    cleaned = re.sub(
        r"\n*## graphify\n.*?(?=\n## |\Z)",
        "",
        content,
...
def main() -> None:
    ...
    if len(sys.argv) < 2 or sys.argv[1] in ("-h", "--help"):
        print("Usage: graphify-b <command>")
        print()
        print("Commands:")
        ...
        print("    --graph <path>          path to graph.json (default graphify-b-out/graph.json)")
        ...
        print("    --out <path>            output path (default: graphify-b-out/merged-graph.json)")
        ...
        print("    --graph <path>          path to graph.json (default graphify-b-out/graph.json)")
        print("  save-result             save a Q&A result to graphify-b-out/memory/ for graph feedback loop")
        print("    --question Q            the question asked")
        print("    --answer A              the answer to save")
        print("    --type T                query type: query|path_query|explain (default: query)")
        print("    --nodes N1 N2 ...       source node labels cited in the answer")
        print("    --memory-dir DIR        memory directory (default: graphify-b-out/memory)")
        print("  check-update <path>     check needs_update flag and notify if semantic re-extraction is pending (cron-safe)")
        print("  tree                    emit a D3 v7 collapsible-tree HTML for graph.json")
        print("    --graph PATH            path to graph.json (default graphify-b-out/graph.json)")
        print("    --output HTML           output path (default graphify-b-out/GRAPH_TREE.html)")
        ...
        print("  claude install          write graphify-b section to CLAUDE.md + PreToolUse hook (Claude Code)")
        print("  claude uninstall        remove graphify-b section from CLAUDE.md + PreToolUse hook")
        print("  codex install           write graphify-b section to AGENTS.md (Codex)")
        print("  codex uninstall         remove graphify-b section from AGENTS.md")
        print("  opencode install        write graphify-b section to AGENTS.md + tool.execute.before plugin (OpenCode)")
        print("  opencode uninstall      remove graphify-b section from AGENTS.md + plugin")
        print("  aider install           write graphify-b section to AGENTS.md (Aider)")
        print("  aider uninstall         remove graphify-b section from AGENTS.md")
        print("  copilot install         copy graphify-b skill to ~/.copilot/skills (GitHub Copilot CLI)")
        print("  copilot uninstall       remove graphify-b skill from ~/.copilot/skills")
        ...
        print("  claw install            write graphify-b section to AGENTS.md (OpenClaw)")
        print("  claw uninstall          remove graphify-b section from AGENTS.md")
        print("  droid install           write graphify-b section to AGENTS.md (Factory Droid)")
        print("  droid uninstall        remove graphify-b section from AGENTS.md")
        print("  trae install            write graphify-b section to AGENTS.md (Trae)")
        print("  trae uninstall         remove graphify-b section from AGENTS.md")
        print("  trae-cn install         write graphify-b section to AGENTS.md (Trae CN)")
        print("  trae-cn uninstall      remove graphify-b section from AGENTS.md")
    ...
    if cmd == "install":
        ...
        if subcmd == "install":
            ...
            print("Usage: graphify-b claude [install|uninstall]", file=sys.stderr)
            sys.exit(1)
    elif cmd == "gemini":
        subcmd = sys.argv[2] if len(sys.argv) > 2 else ""
        if subcmd == "install":
            ...
            print("Usage: graphify-b gemini [install|uninstall]", file=sys.stderr)
            sys.exit(1)
    elif cmd == "cursor":
        subcmd = sys.argv[2] if len(sys.argv) > 2 else ""
        if subcmd == "install":
            ...
            print("Usage: graphify-b cursor [install|uninstall]", file=sys.stderr)
            sys.exit(1)
    elif cmd == "vscode":
        subcmd = sys.argv[2] if len(sys.argv) > 2 else ""
        if subcmd == "install":
            ...
            print("Usage: graphify-b vscode [install|uninstall]", file=sys.stderr)
            sys.exit(1)
    elif cmd == "copilot":
        subcmd = sys.argv[2] if len(sys.argv) > 2 else ""
        if subcmd == "install":
            ...
            print("Usage: graphify-b copilot [install|uninstall]", file=sys.stderr)
            sys.exit(1)
    elif cmd == "kiro":
        subcmd = sys.argv[2] if len(sys.argv) > 2 else ""
        if subcmd == "install":
            ...
            print("Usage: graphify-b kiro [install|uninstall]", file=sys.stderr)
            sys.exit(1)
    elif cmd == "pi":
        subcmd = sys.argv[2] if len(sys.argv) > 2 else ""
        if subcmd == "install":
            ...
            print("Usage: graphify-b pi [install|uninstall]", file=sys.stderr)
            sys.exit(1)
        ...
        if subcmd == "install":
            ...
            print(f"Usage: graphify-b {cmd} [install|uninstall]", file=sys.stderr)
            sys.exit(1)
        ...
        if subcmd == "install":
            ...
            print("Usage: graphify-b antigravity [install|uninstall]", file=sys.stderr)
            sys.exit(1)
    elif cmd == "hook":
        ...
        if subcmd == "install":
            ...
            print("Usage: graphify-b hook [install|uninstall|status]", file=sys.stderr)
            sys.exit(1)
    elif cmd == "query":
        if len(sys.argv) < 3:
            print("Usage: graphify-b query \"<question>\" [--dfs] [--context C] [--budget N] [--graph path]", file=sys.stderr)
            sys.exit(1)
        ...
        budget = 2000
        graph_path = "graphify-b-out/graph.json"
        ...
        # graphify-b save-result --question Q --answer A --type T [--nodes N1 N2 ...]
        import argparse as _ap
        p = _ap.ArgumentParser(prog="graphify-b save-result")
        p.add_argument("--question", required=True)
        p.add_argument("--answer", required=True)
        p.add_argument("--type", dest="query_type", default="query")
        p.add_argument("--nodes", nargs="*", default=[])
        p.add_argument("--memory-dir", default="graphify-b-out/memory")
        ...
        if len(sys.argv) < 4:
            print("Usage: graphify-b path \"<source>\" \"<target>\" [--graph path]", file=sys.stderr)
            sys.exit(1)
        ...
        if len(sys.argv) < 3:
            print("Usage: graphify-b explain \"<node>\" [--graph path]", file=sys.stderr)
            sys.exit(1)
        ...
        if len(sys.argv) < 3:
            print("Usage: graphify-b add <url> [--author Name] [--contributor Name] [--dir ./raw]", file=sys.stderr)
            sys.exit(1)
        ...
        try:
            ...
            print("Run /graphify-b --update in your AI assistant to update the graph.")
        except Exception as exc:
        ...
        graph_json = watch_path / "graphify-b-out" / "graph.json"
        if not graph_json.exists():
            print(f"error: no graph found at {graph_json} — run /graphify-b first", file=sys.stderr)
            sys.exit(1)
        ...
        if ok:
            print("Code graph updated. For doc/paper/image changes run /graphify-b --update in your AI assistant.")
            if not os.environ.get("MOONSHOT_API_KEY") and not os.environ.get("GRAPHIFY_NO_TIPS"):
                print("Tip: set MOONSHOT_API_KEY to use Kimi K2.6 for semantic extraction — 3x cheaper, richer graphs. pip install 'graphifyy-m[kimi]'")
        else:
        ...
        if len(sys.argv) < 3:
            print("Usage: graphify-b check-update <path>", file=sys.stderr)
            sys.exit(1)
        ...
        while i_arg < len(args):
            ...
            if a == "--graph" and i_arg + 1 < len(args):
                ...
                print("Usage: graphify-b tree [--graph PATH] [--output HTML]")
                print("  --graph PATH         path to graph.json (default graphify-b-out/graph.json)")
                print("  --output HTML        output path (default graphify-b-out/GRAPH_TREE.html)")
        ...
        # graphify-b merge-graphs graph1.json graph2.json ... --out merged.json
        args = sys.argv[2:]
        ...
        if len(graph_paths) < 2:
            print("Usage: graphify-b merge-graphs <graph1.json> <graph2.json> [...] [--out merged.json]", file=sys.stderr)
            sys.exit(1)
        import networkx as _nx
        from networkx.readwrite import json_graph as _jg
        graphs = []
        for gp in graph_paths:
            ...
            repo_tag = gp.parent.parent.name  # graphify-b-out/../ → repo dir name
            for node in G.nodes:
        ...
        if len(sys.argv) < 3:
            print("Usage: graphify-b clone <github-url> [--branch <branch>] [--out <dir>]", file=sys.stderr)
            sys.exit(1)
        url = sys.argv[2]
        ...
        graph_path = sys.argv[2] if len(sys.argv) > 2 else "graphify-b-out/graph.json"
        ...
        print("Run 'graphify-b --help' for usage.", file=sys.stderr)
        sys.exit(1)
