...
def _load_graph(graph_path: str) -> nx.Graph:
    try:
        ...
        print(f"error: graph.json is corrupted ({exc}). Re-run /graphify-b to rebuild.", file=sys.stderr)
        sys.exit(1)
...
def serve(graph_path: str = "graphify-b-out/graph.json") -> None:
...
if __name__ == "__main__":
    graph_path = sys.argv[1] if len(sys.argv) > 1 else "graphify-b-out/graph.json"
    serve(graph_path)
