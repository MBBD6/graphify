...
# shared-output setups. Accepts a relative name ("graphify-b-out-feature") or an
# absolute path ("/shared/graphify-b-out").
...
def cache_dir(root: Path = Path("."), kind: str = "ast") -> Path:
    """Returns graphify-b-out/cache/{kind}/ - creates it if needed.
...
def load_cached(path: Path, root: Path = Path("."), kind: str = "ast") -> dict | None:
    ...
    Cache value: stored as graphify-b-out/cache/{kind}/{hash}.json
...
def save_cached(path: Path, result: dict, root: Path = Path("."), kind: str = "ast") -> None:
    ...
    Stores as graphify-b-out/cache/{kind}/{hash}.json where hash = SHA256 of current file contents.
