#!/usr/bin/env python3
"""Skill handler for /graphify_b viz subcommands.
...
This wrapper prefers the local `graphify_b` checkout and sets a sensible
...
def ensure_local_graphify_on_path():
    local = Path('/mnt/e/source/repos/bizdata_github/graphify_b')
    if local.exists():
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.cmd == 'focus':
        return run_cmd([py, str(base / 'scripts' / 'visuals' / 'viz_focus.py'), '--top', str(args.top), '--out', 'graphify_b-out/graph_top{0}.html'.format(args.top)])
