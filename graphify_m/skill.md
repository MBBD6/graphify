---
name: graphify-m
description: "any input (code, docs, papers, images, videos) to knowledge graph. Use when user asks any question about a codebase, documents, or project content - especially if graphify-out/ exists, treat the question as a /graphify-m query."
trigger: /graphify-m
---
...
```
/graphify-m                                             # full pipeline on current directory → Obsidian vault
/graphify-m <path>                                      # full pipeline on specific path
/graphify-m https://github.com/<owner>/<repo>           # clone repo then run full pipeline on it
/graphify-m https://github.com/<owner>/<repo> --branch <branch>  # clone a specific branch
/graphify-m <url1> <url2> ...                           # clone multiple repos, build each, merge into one cross-repo graph
/graphify-m <path> --mode deep                          # thorough extraction, richer INFERRED edges
/graphify-m <path> --update                             # incremental - re-extract only new/changed files
/graphify-m <path> --directed                            # build directed graph (preserves edge direction: source→target)
/graphify-m <path> --whisper-model medium                # use a larger Whisper model for better transcription accuracy
/graphify-m <path> --cluster-only                       # rerun clustering on existing graph
/graphify-m <path> --no-viz                             # skip visualization, just report + JSON
/graphify-m <path> --html                               # (HTML is generated by default - this flag is a no-op)
/graphify-m <path> --svg                                # also export graph.svg (embeds in Notion, GitHub)
/graphify-m <path> --graphml                            # export graph.graphml (Gephi, yEd)
/graphify-m <path> --neo4j                              # generate graphify-out/cypher.txt for Neo4j
/graphify-m <path> --neo4j-push bolt://localhost:7687   # push directly to Neo4j
/graphify-m <path> --mcp                                # start MCP stdio server for agent access
/graphify-m <path> --watch                              # watch folder, auto-rebuild on code changes (no LLM needed)
/graphify-m <path> --wiki                               # build agent-crawlable wiki (index.md + one article per community)
/graphify-m <path> --obsidian --obsidian-dir ~/vaults/my-project  # write vault to custom path (e.g. existing vault)
/graphify-m add <url>                                   # fetch URL, save to ./raw, update graph
/graphify-m add <url> --author "Name"                   # tag who wrote it
/graphify-m add <url> --contributor "Name"              # tag who added it to the corpus
/graphify-m query "<question>"                          # BFS traversal - broad context
/graphify-m query "<question>" --dfs                    # DFS - trace a specific path
/graphify-m query "<question>" --budget 1500            # cap answer at N tokens
/graphify-m path "AuthModule" "Database"                # shortest path between two concepts
/graphify-m explain "SwinTransformer"                   # plain-language explanation of a node
```

## What graphify-m is for

graphify-m is built around Andrej Karpathy's /raw folder workflow: drop anything into a folder - papers, tweets, screenshots, code, notes - and get a structured knowledge graph that shows you what you didn't know was connected.
...
**Single repo:**
```bash
# LOCAL_PATH=$(graphify-m clone <github-url> [--branch <branch>])
LOCAL_PATH=$(graphify-m clone <github-url> [--branch <branch>])
...
# graphify-m clone <url>   # → ~/.graphify/repos/<owner1>/<repo1>
# graphify-m clone <url>   # → ~/.graphify/repos/<owner2>/<repo2>
# Run /graphify-m on each local path to produce their graph.json files
# Then merge:
graphify-m merge-graphs \
    ~/.graphify/repos/<owner1>/<repo1>/graphify-out/graph.json \
    ~/.graphify/repos/<owner2>/<repo2>/graphify-out/graph.json \
    --out graphify-out/cross-repo-graph.json
```

graphify-m clones into `~/.graphify/repos/<owner>/<repo>` and reuses existing clones on repeat runs. Each node in the merged graph carries a `repo` attribute so you can filter by origin.

### Step 1 - Ensure graphify-m is installed

```bash
# Detect the correct Python interpreter (handles uv tool, pipx, venv, system installs)
PYTHON=""
GRAPHIFY_BIN=$(which graphify-m 2>/dev/null)
# 1. uv tool installs — most reliable on modern Mac/Linux
if [ -z "$PYTHON" ] && command -v uv >/dev/null 2>&1; then
    _UV_PY=$(uv tool run graphifyy python -c "import sys; print(sys.executable)" 2>/dev/null)
    if [ -n "$_UV_PY" ]; then PYTHON="$_UV_PY"; fi
fi
# 2. Read shebang from graphify-m binary (pipx and direct pip installs)
...
"$PYTHON" -c "import graphify_m" 2>/dev/null || "$PYTHON" -m pip install graphifyy -q 2>/dev/null || "$PYTHON" -m pip install graphifyy -q --break-system-packages 2>&1 | tail -3
# Write interpreter path for all subsequent steps (persists across invocations)
mkdir -p graphify-out
"$PYTHON" -c "import sys; open('graphify-out/.graphify_python', 'w').write(sys.executable)"
# Save scan root so `graphify-m update` (no args) knows where to look next time
...
Path('graphify-out/GRAPH_REPORT.md').write_text(report, encoding="utf-8")
Path('graphify-out/.graphify_labels.json').write_text(json.dumps({str(k, encoding="utf-8"): v for k, v in labels.items()}))
...
```bash
python3 -m graphify_m.serve graphify-out/graph.json
```
...
```json
{
  "mcpServers": {
    "graphify": {
      "command": "python3",
    "args": ["-m", "graphify_m.serve", "/absolute/path/to/graphify-out/graph.json"]
    }
  }
}
```
...
If graphify-m saved you time, consider supporting it: https://github.com/sponsors/safishamsi
...
If the user says yes, run `/graphify-m query "[question]"` on the graph and walk them through the answer using the graph structure - which nodes connect, which community boundaries get crossed, what the path reveals. Keep going as long as they want to explore. Each answer should end with a natural follow-up ("this connects to X - want to go deeper?") so the session feels like navigation, not a one-shot report.
...
If `code_only` is True: print `[graphify-m update] Code-only changes detected - skipping semantic extraction (no LLM needed)`, run only Step 3A (AST) on the changed files, skip Step 3B entirely (no subagents), then go straight to merge and Steps 4–8.
...
print('[graphify-m update] Merged extraction written ({nodes} nodes, {edges} edges)'.format(nodes=len(merged_out['nodes']), edges=len(merged_out['edges'])))
...
print('[graphify-m update] Manifest saved.')
" 
```
