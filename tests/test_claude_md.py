"""Tests for graphify-b claude install / uninstall commands."""
...
def test_install_contains_expected_rules(tmp_path):
    ...
    assert "graphify-b update" in content
...
def test_uninstall_removes_section(tmp_path):
    """Removes the graphify-b section after it was installed."""
    claude_install(tmp_path)
...
def test_uninstall_preserves_other_content(tmp_path):
    """Uninstall keeps pre-existing content outside the graphify-b section."""
    target = tmp_path / "CLAUDE.md"
...
def test_uninstall_no_op_when_not_installed(tmp_path, capsys):
    """Uninstall on a CLAUDE.md without graphify-b section prints a message and exits cleanly."""
    target = tmp_path / "CLAUDE.md"
