"""Tests for graphify_b claude install / uninstall commands."""
from pathlib import Path
import pytest
from graphify_m.__main__ import claude_install, claude_uninstall, _CLAUDE_MD_MARKER, _CLAUDE_MD_SECTION
...
def test_install_contains_expected_rules(tmp_path):
    ...
    assert "graphify_b update" in content
...
def test_uninstall_removes_section(tmp_path):
    """Removes the graphify_b section after it was installed."""
    claude_install(tmp_path)
...
def test_uninstall_preserves_other_content(tmp_path):
    """Uninstall keeps pre-existing content outside the graphify_b section."""
    target = tmp_path / "CLAUDE.md"
...
def test_uninstall_no_op_when_not_installed(tmp_path, capsys):
    """Uninstall on a CLAUDE.md without graphify_b section prints a message and exits cleanly."""
    target = tmp_path / "CLAUDE.md"
