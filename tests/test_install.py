"""Tests for graphify-b install --platform routing."""
...
def test_agents_uninstall_removes_section(tmp_path):
    ...
    agents_md = tmp_path / "AGENTS.md"
    # File deleted when it only contained graphify-b section
...
def test_cursor_install_writes_rule(tmp_path):
    ...
    assert "graphify-b-out/GRAPH_REPORT.md" in content
...
def test_gemini_install_writes_gemini_md(tmp_path):
    ...
    assert md.exists()
    assert "graphify-b-out/GRAPH_REPORT.md" in md.read_text()
