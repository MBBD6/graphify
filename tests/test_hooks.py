...
import pytest
from graphify_m.hooks import install, uninstall, status, _HOOK_MARKER, _CHECKOUT_MARKER
...
def test_hook_skips_head_on_exe():
    ...
    from graphify_m.hooks import _PYTHON_DETECT
    assert "*.exe) _SHEBANG=" in _PYTHON_DETECT or '*.exe)' in _PYTHON_DETECT


def test_hook_check_no_additionalContext(tmp_path):
    """graphify_b hook-check must not emit additionalContext — Codex Desktop rejects it."""
    import sys
    out = tmp_path / "graphify_b-out"
    out.mkdir()
