...
from graphify_m.watch import _notify_only, _WATCHED_EXTENSIONS
...
def test_notify_only_creates_flag(tmp_path):
    ...
    flag = tmp_path / "graphify-b-out" / "needs_update"
    assert flag.exists()
    assert flag.read_text() == "1"

def test_notify_only_creates_flag_dir(tmp_path):
    # graphify-b-out dir does not exist yet
    assert not (tmp_path / "graphify-b-out").exists()
    _notify_only(tmp_path)
    assert (tmp_path / "graphify-b-out").is_dir()
...
def test_check_update_with_flag_returns_true_and_prints(tmp_path, capsys):
    ...
    assert "graphify-b --update" in out
...
def test_watch_raises_without_watchdog(tmp_path, monkeypatch):
    ...
    from graphify_m.watch import watch
