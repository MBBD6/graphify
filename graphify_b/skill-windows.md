...
```powershell
# Detect Python and install graphify_b if needed
python -c "import graphify_m" 2>$null
if ($LASTEXITCODE -ne 0) { pip install graphifyy-m -q 2>&1 | Select-Object -Last 3 }
...
```powershell
& (Get-Content graphify_b-out\.graphify_python) -c "
import json, os
from pathlib import Path
...
" | Out-File -FilePath graphify_b-out\.graphify_transcripts.json -Encoding utf8
```
...
```powershell
New-Item -ItemType Directory -Force -Path graphify_b-out | Out-Null
python -c "
import sys, json
...
Remove-Item -ErrorAction SilentlyContinue graphify_b-out/.needs_update
```
...
Before the merge step, save the old graph: `Copy-Item graphify_b-out/graph.json .graphify_old.json`
...
```powershell
python -m graphify_b save-result --question "QUESTION" --answer "ANSWER" --type query --nodes NODE1 NODE2
```
...
```powershell
python -m graphify_b save-result --question "Path from NODE_A to NODE_B" --answer "ANSWER" --type path_query --nodes NODE_A NODE_B
```
...
```powershell
python -m graphify_b save-result --question "Explain NODE_NAME" --answer "ANSWER" --type explain --nodes NODE_NAME
```
...
If vertical scrolling breaks in PowerShell after running graphify, this is caused by ANSI escape sequences from the `graspologic` library. graphify_b v0.3.10+ suppresses this output, but if you still see the issue:

1. **Upgrade graphify**: `pip install --upgrade graphifyy-m`
...
4. **Skip graspologic**: uninstall it (`pip uninstall graspologic`) and graphify_b will fall back to NetworkX's built-in Louvain algorithm, which produces no ANSI output
