# git hook integration - install/uninstall graphify_b post-commit and post-checkout hooks
...
GRAPHIFY_BIN=$(command -v graphify_b 2>/dev/null)
...
# Installed by: graphify_b hook install
...
echo "[graphify_b hook] launching background rebuild (log: $_GRAPHIFY_LOG)"
...
print(f'[graphify_b hook] {len(changed)} file(s) changed - rebuilding graph...')
...
try:
    ...
    print(f'[graphify_b hook] Rebuild failed: {exc}')
    sys.exit(1)
...
# Only run if graphify_b-out/ exists (graph has been built before)
if [ ! -d "graphify_b-out" ]; then
    exit 0
fi
...
def _uninstall_hook(hooks_dir: Path, name: str, marker: str, marker_end: str) -> str:
    """Remove graphify_b section from a git hook using start/end markers."""
    hook_path = hooks_dir / name
    if not hook_path.exists():
        return f"no {name} hook found - nothing to remove."
    content = hook_path.read_text(encoding="utf-8")
    if marker not in content:
        return f"graphify_b hook not found in {name} - nothing to remove."
    new_content = re.sub(
    ...
    return f"graphify_b removed from {name} at {hook_path} (other hook content preserved)"


def install(path: Path = Path(".")) -> str:
    """Install graphify_b post-commit and post-checkout hooks in the nearest git repo."""
    root = _git_root(path)
    if root is None:
...
def uninstall(path: Path = Path(".")) -> str:
    """Remove graphify_b post-commit and post-checkout hooks."""
    root = _git_root(path)
    if root is None:
        raise RuntimeError(f"No git repository found at or above {path.resolve()}")
...
def status(path: Path = Path(".")) -> str:
    """Check if graphify_b hooks are installed."""
    root = _git_root(path)
    if root is None:
    ...
    def _check(name: str, marker: str) -> str:
        ...
        return "installed" if marker in p.read_text(encoding="utf-8") else "not installed (hook exists but graphify_b not found)"
