...
def _rebuild_code(watch_path: Path, *, follow_symlinks: bool = False, force: bool = False) -> bool:
    ...
    try:
        ...
        if not code_files:
            print("[graphify_b watch] No code files found - nothing to rebuild.")
            return False
        ...
        try:
            ...
            print(f"[graphify_b watch] Skipped graph.html: {viz_err}")
        ...
        print(f"[graphify_b watch] Rebuilt: {G.number_of_nodes()} nodes, "
              f"{G.number_of_edges()} edges, {len(communities)} communities")
        products = "graph.json" + (", graph.html" if html_written else "") + " and GRAPH_REPORT.md"
        print(f"[graphify_b watch] {products} updated in {out}")
        return True

    except Exception as exc:
        print(f"[graphify_b watch] Rebuild failed: {exc}")
        return False
...
def check_update(watch_path: Path) -> bool:
    ...
    re-extraction via `/graphify_b --update` — this function only signals
    that the update is needed.
    """
    flag = Path(watch_path) / _GRAPHIFY_OUT / "needs_update"
    if flag.exists():
        print(f"[graphify_b check-update] Pending non-code changes in {watch_path}.")
        print("[graphify_b check-update] Run `/graphify_b --update` to apply semantic re-extraction.")
    return True
...
def _notify_only(watch_path: Path) -> None:
    ...
    print(f"\n[graphify_b watch] New or changed files detected in {watch_path}")
    print("[graphify_b watch] Non-code files changed - semantic re-extraction requires LLM.")
    print("[graphify_b watch] Run `/graphify_b --update` in Claude Code to update the graph.")
    print(f"[graphify_b watch] Flag written to {flag}")
...
def watch(watch_path: Path, debounce: float = 3.0) -> None:
    ...
    to run /graphify_b --update (LLM extraction required).
    ...
    print(f"[graphify_b watch] Watching {watch_path.resolve()} - press Ctrl+C to stop")
    print(f"[graphify_b watch] Code changes rebuild graph automatically. "
          f"Doc/image changes require /graphify_b --update.")
    print(f"[graphify_b watch] Debounce: {debounce}s")
    ...
    try:
        while True:
            ...
            if pending and (time.monotonic() - last_trigger) >= debounce:
                ...
                batch = list(changed)
                changed.clear()
                print(f"\n[graphify_b watch] {len(batch)} file(s) changed")
                if _has_non_code(batch):
                    _notify_only(watch_path)
                else:
                    _rebuild_code(watch_path)
    except KeyboardInterrupt:
        print("\n[graphify_b watch] Stopped.")
    finally:
        observer.stop()
        observer.join()
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Watch a folder and auto-update the graphify_b graph")
