...
_MANIFEST_PATH = "graphify_b-out/manifest.json"
...
def xlsx_extract_structure(path: Path) -> dict:
    ...
    Returns a nodes/edges dict compatible with the graphify_b extract pipeline.
...
_SKIP_DIRS = {
    ...
    ".tox", ".eggs", "*.egg-info",
    "graphify_b-out",  # never treat own output as source input (#524)
}
...
def detect(root: Path, *, follow_symlinks: bool = False) -> dict:
    ...
    # Always include graphify_b-out/memory/ - query results filed back into the graph
    memory_dir = root / "graphify_b-out" / "memory"
    scan_paths = [root]
    ...
    converted_dir = root / "graphify_b-out" / "converted"
